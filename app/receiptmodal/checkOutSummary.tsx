...
interface CheckoutSummaryProps {
  ...
  setPaymentReceived: (val: number) => void;
  cookingRequest: string;
  setCookingRequest: (val: string) => void;
  isEditingRequest: boolean;
  setIsEditingRequest: (val: boolean) => void;
  onPlaceOrder: () => void;
  ...
  <section className="shrink-0 p-5 bg-white border-t border-[#DCC7AA] rounded-t-[2.5rem] shadow-[0_-10px_40px_rgba(0,0,0,0.1)]">
    ...
    <div className="bg-[#F5E6CA]/20 rounded-2xl p-4 border border-[#DCC7AA]/20 space-y-2">
      ...
      <div className="flex justify-between items-end border-t border-dashed border-[#DCC7AA] pt-3 pb-1">
        <span className="text-[10px] font-black text-[#4B3832] uppercase italic tracking-widest">
          Total Due
        </span>
        <span className="text-xl font-black text-[#4B3832] leading-none tracking-tighter">
          ₱{finalTotal.toFixed(2)}
        </span>
      </div>
      {/*payment received input section*/}
      <div className="flex items-center justify-between gap-3 pt-1">
        <span className="text-[10px] font-black text-[#DCC7AA] uppercase tracking-widest">
          Cash Received
        </span>
        <input
          type="number"
      ...
      <div className="flex justify-between items-center pt-1">
        ...
        <span className="text-sm font-black text-[#6F4E37]">
          ₱{change.toFixed(2)}
        </span>
      </div>
    </div>
