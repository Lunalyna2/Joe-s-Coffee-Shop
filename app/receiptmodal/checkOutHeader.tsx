  ...
  <section className="p-5 shrink-0 bg-[#4B3832] text-white z-20">
    <div className="flex justify-between items-center mb-4">
      ...
      <button
        ...
        className="text-[#DCC7AA] hover:bg-white/10 rounded-full p-1.5 transition"
      >
    ...
    <div className="grid grid-cols-2 gap-2 bg-[#F5E6CA]/10 rounded-xl p-1 border border-white/10 mb-4">
      {["DINE IN", "TAKE OUT"].map((type) => (
        <button
          key={type}
          onClick={() =>
            setOrderType(type.toLowerCase().replace(" ", "_") as OrderType)
          }
          className={`text-center py-2.5 rounded-lg text-[8px] font-black tracking-[0.2em] transition-all ${
            orderType === type.toLowerCase().replace(" ", "_")
              ? "bg-[#DCC7AA] text-[#4B3832] shadow-md"
    ...
    <div className="relative group">
      <User
        className="absolute left-4 top-1/2 -translate-y-1/2 text-[#4B3832]/40"
        size={14}
      />
      <input
        type="text"
