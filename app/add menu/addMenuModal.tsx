...
export function AddMenuModal({ menuItems, onClose, onMenuUpdate }: AddMenuModalProps) {
  ...
  const categories = ["NON-COFFEE", "COFFEE-BASED", "PASTA & BURGERS", "DESSERTS & PASTRIES"];
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setFormData({ name: '', price: '', category: 'COFFEE-BASED', available: 'true' });
  };
      ...
      <div className="relative w-full max-w-5xl bg-[#F5E6CA] rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col md:flex-row h-[85vh] border-4 border-[#4B3832]">
        ...
        <div className="w-full md:w-1/3 bg-white p-8 border-r-2 border-[#4B3832]/10 overflow-y-auto">
          <div className="mb-8">
            ...
            <p className="text-[10px] font-bold text-[#DCC7AA] uppercase tracking-[0.2em]">Menu Configuration</p>
          </div>

          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              <label className="text-[9px] font-black text-[#4B3832] uppercase tracking-widest mb-2 block">Item Name</label>
              <input 
                required
                name="name"
                value={formData.name}
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="text-[9px] font-black text-[#4B3832] uppercase tracking-widest mb-2 block">Price (₱)</label>
                <input 
                  required
                  type="number"
                  step="0.01"
              ...
              <div>
                <label className="text-[9px] font-black text-[#4B3832] uppercase tracking-widest mb-2 block">Status</label>
                <select 
                  name="available"
            ...
            <div>
              <label className="text-[9px] font-black text-[#4B3832] uppercase tracking-widest mb-2 block">Category</label>
              <select 
                name="category"
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          <div className="p-8 flex justify-between items-center bg-[#F5E6CA] border-b border-[#4B3832]/5">
            <div>
              <h3 className="text-xl font-black text-[#4B3832] uppercase italic">Inventory List</h3>
              <p className="text-[13px] font-bold text-[#4B3832]">{items.length} Items total</p>
            </div>
            <button onClick={onClose} className="p-2 hover:bg-[#4B3832]/10 rounded-full transition-colors">
          ...
          <div className="flex-1 overflow-y-auto p-8 space-y-3 no-scrollbar">
            {items.length === 0 ? (
              <div className="text-center py-20 opacity-20">
                ...
                <p className="font-black text-[#4B3832] uppercase tracking-widest">Menu is Empty</p>
              </div>
            ) : (
              items.map((item) => (
                <div key={item.id} className="flex items-center justify-between bg-white/60 p-4 rounded-2xl border border-[#DCC7AA]/30 group hover:border-[#4B3832]/30 transition-all">
                  <div className="flex items-center gap-4">
                    <div className={`w-10 h-10 rounded-lg flex items-center justify-center transition-colors ${item.available ? 'bg-[#4B3832] text-[#DCC7AA]' : 'bg-gray-200 text-gray-400'}`}>
                      <Package size={18} />
                    </div>
                    <div>
                      <h4 className={`text-[11px] font-black uppercase italic ${item.available ? 'text-[#4B3832]' : 'text-gray-400 line-through'}`}>
                        {item.name}
                      </h4>
